...
const ShareCodePage: React.FC = () => {
    ...
    const handleShare = async () => {
        const link = `${window.location.origin}${window.location.pathname}?code=${LZString.compressToEncodedURIComponent(sharedCode)}`;
    
        try {
            const response = await fetch(`https://api.tinyurl.com/v1/create?url=${encodeURIComponent(link)}&apikey=${process.env.TOKEN}`, {
                method: 'POST',
                headers: {
            ...
            if (data && data.data && data.data.tiny_url) {
                ...
                showToast("Shortened link copied to clipboard: " + data.data.tiny_url);
            } else {
